  ...
  return (
    <div className="space-y-6">
      <div className="text-center py-4">
        ...
        <p className="text-[#8A7E78] text-sm mt-1">생년월일시를 입력하면 정확한 사주를 분석합니다</p>
      </div>
        ...
        <div className="space-y-5">
          ...
          <div className="card">
            ...
            <div className="grid grid-cols-2 gap-2 text-sm">
              <div className="bg-[#EFDED5] rounded-lg p-2"><span className="text-[#8A7E78]">생년월일:</span> <span className="font-medium">{result.birthDate}</span></div>
              <div className="bg-[#EFDED5] rounded-lg p-2"><span className="text-[#8A7E78]">출생시간:</span> <span className="font-medium">{result.birthTime || "미입력"}</span></div>
              <div className="bg-[#EFDED5] rounded-lg p-2"><span className="text-[#8A7E78]">성별:</span> <span className="font-medium">{result.gender}</span></div>
              <div className="bg-[#EFDED5] rounded-lg p-2"><span className="text-[#8A7E78]">나이:</span> <span className="font-medium">{result.age}세</span></div>
              <div className="bg-[#EFDED5] rounded-lg p-2"><span className="text-[#8A7E78]">띠:</span> <span className="font-medium">{result.ddi?.emoji} {result.ddi?.name} ({result.ddi?.hanja})</span></div>
              <div className="bg-[#EFDED5] rounded-lg p-2"><span className="text-[#8A7E78]">일간:</span> <span className="font-medium">{result.dayGan} ({result.mainElement})</span></div>
              {result.strength && <div className="bg-[#EFDED5] rounded-lg p-2"><span className="text-[#8A7E78]">신강/신약:</span> <span className="font-medium">{result.strength}{result.strengthScore ? ` (${result.strengthScore}점)` : ""}</span></div>}
            {result.gyeok && <div className="bg-gradient-to-r from-[#EDE8F5] to-[#EDE9FE] rounded-xl p-3 border border-[#DDD6FE]"><span className="text-xs text-[#7C3AED] font-bold block mb-1">격국 (格局)</span><span className="text-sm font-bold text-[#3D3338]">{result.gyeok}</span></div>}
            {result.yongshin && <div className="bg-gradient-to-r from-[#E8E4F0] to-[#FDE68A] rounded-xl p-3 border border-[#FCD34D] col-span-2"><span className="text-xs text-[#B45309] font-bold block mb-1">용신 (用神)</span><span className="text-sm font-bold text-[#3D3338]">{result.yongshin}</span></div>}
            {result.gongmang && <div className="bg-[#EFDED5] rounded-xl p-3 border border-[#E8E0D8] col-span-2"><span className="text-xs text-[#8A7E78] font-bold block mb-1">공망 (空亡)</span><span className="text-sm font-bold text-[#3D3338]">{result.gongmang}</span></div>}
            </div>
          </div>
          ...
          <div className="card">
            ...
            <div className="grid grid-cols-4 gap-2">
              {(["hour", "day", "month", "year"] as const).map((key) => {
                ...
                if (!p) return null;
                return (
                  <div key={key} className="rounded-xl border-2 p-3 text-center" style={{ borderColor: p.skyColor || "#D9C8C0" }}>
                    <div className="text-[10px] text-[#8A7E78] mb-1">{p.label}</div>
                    <div className="text-xs text-[#8A7E78]">{p.tenGodSky || ""}</div>
                    <div className="text-2xl my-1">{p.skyEmoji}</div>
                    <div className="font-bold text-lg" style={{ color: p.skyColor }}>{p.sky}</div>
                    <div className="text-xs text-[#8A7E78]">{p.skyKo} ({p.skyElement})</div>
                    <hr className="my-2 border-[#D9C8C0]" />
                    <div className="text-xs text-[#8A7E78]">{p.tenGodEarth || ""}</div>
          ...
          <div className="card">
            ...
            <div className="grid grid-cols-4 gap-2 text-center text-sm">
              {(["hour", "day", "month", "year"] as const).map((key) => {
                ...
                return (
                  <div key={key} className="bg-[#EFDED5] rounded-xl p-3">
              ...
              <div className="flex gap-2 justify-center mb-4">
                {Object.entries(result.ohaengCount).map(([name, count]) => (
                  <div key={name} className="flex-1 text-center rounded-xl p-3" style={{ backgroundColor: ohaengColor(name) + "18", border: `2px solid ${ohaengColor(name)}40` }}>
                    <div className="text-xs text-[#5A4E48]">{name}</div>
              ...
              {result.ohaengAnalysis && result.ohaengAnalysis.length > 0 && (
                <div className="space-y-2">
                  {result.ohaengAnalysis.filter(o => o.count > 0).map((o) => (
                    <div key={o.name} className="bg-[#F5EDE8] rounded-lg p-2 text-xs text-[#5A5A5A]">
            ...
            <div className="card">
              ...
              <div className="flex flex-wrap gap-2 justify-center mb-4">
                {result.sipsinAnalysis.map((s) => (
                  <div key={s.name} className="bg-[#EFDED5] rounded-lg px-3 py-2 text-center">
              ...
              <div className="space-y-2">
                {result.sipsinAnalysis.filter(s => s.desc && s.name !== "(일간)").map((s) => (
                  <div key={s.name} className="bg-[#F5EDE8] rounded-lg p-2 text-xs text-[#5A5A5A]">
              ...
              {result.stemRelations && result.stemRelations.length > 0 && (
                <div className="mb-3">
                  ...
                  {result.stemRelations.map((r, i) => (
                    <div key={i} className="bg-[#F5EDE8] rounded-lg p-2 mb-1 text-xs text-[#5A5A5A]">
          ...
          {/* 종합 해석 */}
          {result.summary && (
            <div className="card bg-gradient-to-br from-[#F2E4DC] to-[#EDE8F5]">
          ...
          {result.interpretation && (
            <div className="card bg-gradient-to-br from-[#F5EDE8] to-[#FFF0E8]">
