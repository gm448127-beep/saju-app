  ...
  return (
    <div className="space-y-6">
      <div className="text-center py-4">
        ...
        <h1 style={{ fontFamily: "Jua, sans-serif" }} className="text-2xl text-[#3D3338] mt-3">사주팔자 분석</h1>
        <p className="text-[#847A80] text-sm mt-1">생년월일시를 입력하면 정확한 사주를 분석합니다</p>
      </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="grid grid-cols-3 gap-3">
            <div>
              ...
              <select value={year} onChange={(e) => setYear(Number(e.target.value))} className="w-full bg-white border-2 border-[#E8E2DC] rounded-xl px-3 py-2.5 text-[#3D3338] text-sm focus:border-[#8B7EC8] outline-none">
            ...
            <div>
              ...
              <select value={month} onChange={(e) => setMonth(Number(e.target.value))} className="w-full bg-white border-2 border-[#E8E2DC] rounded-xl px-3 py-2.5 text-[#3D3338] text-sm focus:border-[#8B7EC8] outline-none">
                {Array.from({ length: 12 }, (_, i) => i + 1).map((m) => (<option key={m} value={m}>{m}월</option>))}
              </select>
            </div>
            <div>
              <label className="block text-xs text-[#9B9B9B] mb-1" style={{ fontFamily: "Jua, sans-serif" }}>일</label>
              <select value={day} onChange={(e) => setDay(Number(e.target.value))} className="w-full bg-white border-2 border-[#E8E2DC] rounded-xl px-3 py-2.5 text-[#3D3338] text-sm focus:border-[#8B7EC8] outline-none">
          ...
          <div>
            ...
            {timeMode === "slot" && (
              <select value={slotValue} onChange={(e) => setSlotValue(Number(e.target.value))} className="w-full bg-white border-2 border-[#E8E2DC] rounded-xl px-3 py-2.5 text-[#3D3338] text-sm focus:border-[#8B7EC8] outline-none">
              ...
              <div className="bg-[#F0EDFA] rounded-xl p-4 space-y-3">
                ...
                <div className="flex items-center gap-3">
                  <div className="flex-1">
                    ...
                    <select value={exactHour} onChange={(e) => setExactHour(Number(e.target.value))} className="w-full bg-white border-2 border-[#D5CFF0] rounded-xl px-3 py-2.5 text-[#3D3338] text-sm focus:border-[#8B7EC8] outline-none">
                  ...
                  <div className="flex-1">
                    ...
                    <select value={exactMinute} onChange={(e) => setExactMinute(Number(e.target.value))} className="w-full bg-white border-2 border-[#D5CFF0] rounded-xl px-3 py-2.5 text-[#3D3338] text-sm focus:border-[#8B7EC8] outline-none">
      ...
      {result && (
        <div className="space-y-5">
          {/* 기본 정보 */}
          <div className="card">
            <h2 className="text-lg font-bold text-[#3D3338] mb-3" style={{ fontFamily: "Jua, sans-serif" }}>{result.ddi?.emoji} 기본 정보</h2>
            <div className="grid grid-cols-2 gap-2 text-sm">
              <div className="bg-[#F5F0EB] rounded-lg p-2"><span className="text-[#9B9B9B]">생년월일:</span> <span className="font-medium">{result.birthDate}</span></div>
              <div className="bg-[#F5F0EB] rounded-lg p-2"><span className="text-[#9B9B9B]">출생시간:</span> <span className="font-medium">{result.birthTime || "미입력"}</span></div>
              <div className="bg-[#F5F0EB] rounded-lg p-2"><span className="text-[#9B9B9B]">성별:</span> <span className="font-medium">{result.gender}</span></div>
              <div className="bg-[#F5F0EB] rounded-lg p-2"><span className="text-[#9B9B9B]">나이:</span> <span className="font-medium">{result.age}세</span></div>
          ...
          {/* 사주 원국 */}
          <div className="card">
            <h2 className="text-lg font-bold text-[#3D3338] mb-3" style={{ fontFamily: "Jua, sans-serif" }}>📋 사주 원국</h2>
            <div className="grid grid-cols-4 gap-2">
              {(["hour", "day", "month", "year"] as const).map((key) => {
                const p = result.pillars?.[key];
                if (!p) return null;
                return (
                  <div key={key} className="rounded-xl border-2 p-3 text-center" style={{ borderColor: p.skyColor || "#E8E2DC" }}>
                    <div className="text-[10px] text-[#9B9B9B] mb-1">{p.label}</div>
                    <div className="text-xs text-[#847A80]">{p.tenGodSky || ""}</div>
                    <div className="text-2xl my-1">{p.skyEmoji}</div>
                    <div className="font-bold text-lg" style={{ color: p.skyColor }}>{p.sky}</div>
                    <div className="text-xs text-[#9B9B9B]">{p.skyKo} ({p.skyElement})</div>
                    <hr className="my-2 border-[#E8E2DC]" />
                    <div className="text-xs text-[#847A80]">{p.tenGodEarth || ""}</div>
                ...
                return (
                  <div key={key} className="bg-[#F5F0EB] rounded-xl p-3">
                    ...
                    <div className="space-y-1">
                      {h?.yeogi && <div className="text-xs"><span className="text-[#9B9B9B]">여기:</span> {h.yeogi}</div>}
                      {h?.junggi && <div className="text-xs"><span className="text-[#9B9B9B]">중기:</span> {h.junggi}</div>}
                      {h?.jeonggi && <div className="text-xs"><span className="text-[#9B9B9B]">정기:</span> {h.jeonggi}</div>}
          ...
          {result.ohaengCount && (
            <div className="card">
              <h2 className="text-lg font-bold text-[#3D3338] mb-3" style={{ fontFamily: "Jua, sans-serif" }}>🌿 오행 분포</h2>
            ...
            <div className="card">
              ...
              <div className="flex flex-wrap gap-2 justify-center mb-4">
                {result.sipsinAnalysis.map((s) => (
                  <div key={s.name} className="bg-[#F5F0EB] rounded-lg px-3 py-2 text-center">
                    <div className="text-xs text-[#9B9B9B]">{s.name}</div>
              ...
              <div className="space-y-2">
                {result.sipsinAnalysis.filter(s => s.desc && s.name !== "(일간)").map((s) => (
                  <div key={s.name} className="bg-[#FAFAF8] rounded-lg p-2 text-xs text-[#5A5A5A]">
          ...
          {((result.stemRelations && result.stemRelations.length > 0) || (result.branchRelations && result.branchRelations.length > 0)) && (
            <div className="card">
              <h2 className="text-lg font-bold text-[#3D3338] mb-3" style={{ fontFamily: "Jua, sans-serif" }}>🔗 천간·지지 관계</h2>
              {result.stemRelations && result.stemRelations.length > 0 && (
                <div className="mb-3">
                  ...
                  {result.stemRelations.map((r, i) => (
                    <div key={i} className="bg-[#FFF8F0] rounded-lg p-2 mb-1 text-xs text-[#5A5A5A]">
              ...
              {result.branchRelations && result.branchRelations.length > 0 && (
                <div>
                  ...
                  {result.branchRelations.map((r, i) => (
                    <div key={i} className="bg-[#F0F8FF] rounded-lg p-2 mb-1 text-xs text-[#5A5A5A]">
          ...
          {result.salsSummary && result.salsSummary.length > 0 && (
            <div className="card">
              ...
              {result.hyungsin && result.hyungsin.length > 0 && (
                <div className="mt-1 bg-[#FFF0F0] rounded-lg p-2 text-xs"><span className="font-bold text-red-500">흉신:</span> {result.hyungsin.join(", ")}</div>
              )}
            </div>
          )}
            ...
            <div className="card">
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-xs text-center min-w-[600px]">
                  ...
                  <tbody>
                    <tr>
                      <td className="p-1 text-[#9B9B9B]">간지</td>
                      {result.daeun.map((d, i) => (<td key={i} className="p-1 font-bold">{d.gan}{d.ji}</td>))}
                    </tr>
                    <tr>
                      <td className="p-1 text-[#9B9B9B]">한글</td>
                      {result.daeun.map((d, i) => (<td key={i} className="p-1 text-[#847A80]">{d.ganKo}{d.jiKo}</td>))}
                    </tr>
                    <tr>
                    ...
                    <tr>
                      ...
                      {result.daeun.map((d, i) => (<td key={i} className="p-1 text-[#6B6B6B]">{d.stage12 || "-"}</td>))}
                    </tr>
                  </tbody>
                </table>
              </div>
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-xs text-center min-w-[600px]">
                  ...
                  <tbody>
                    ...
                    <tr>
                      <td className="p-1 text-[#9B9B9B]">한글</td>
                      {result.seyun.map((s, i) => (<td key={i} className="p-1 text-[#847A80]">{s.ganKo}{s.jiKo}</td>))}
                    </tr>
                    <tr>
                      <td className="p-1 text-[#9B9B9B]">십성</td>
                    ...
                    <tr>
                      <td className="p-1 text-[#9B9B9B]">12운성</td>
                      {result.seyun.map((s, i) => (<td key={i} className="p-1 text-[#6B6B6B]">{s.stage12 || "-"}</td>))}
                    </tr>
                  </tbody>
                </table>
              </div>
          ...
          {/* 종합 해석 */}
          {result.summary && (
            <div className="card bg-gradient-to-br from-[#F0EDFA] to-[#EDE8F5]">
          ...
          {result.interpretation && (
            <div className="card bg-gradient-to-br from-[#FFF8F0] to-[#FFF0E8]">
