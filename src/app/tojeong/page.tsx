  ...
  return (
    <div className="space-y-6">
      ...
      <div className="text-center py-4">
        ...
        <p className="text-[#847A80] text-sm mt-1">조선시대 토정 이지함 선생의 한 해 운세</p>
      </div>
      ...
      <div className="card">
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="grid grid-cols-3 gap-3">
            <div>
              ...
              <select value={year} onChange={e => setYear(Number(e.target.value))}
                className="w-full bg-white border-2 border-[#E8E2DC] rounded-xl px-3 py-2.5 text-[#2D2D2D] text-sm focus:border-[#8B7EC8] outline-none">
              ...
              {([
                ...
                <button key={mode} type="button" onClick={() => setTimeMode(mode)}
                  className={`flex-1 py-2 rounded-xl text-xs font-medium transition-all ${
                    timeMode === mode
                      ? 'bg-[#8B7EC8] text-white shadow-md'
                      : 'bg-white border-2 border-[#E8E2DC] text-[#9B9B9B]'
                  }`}>{label}</button>
              ))}
            </div>
                ...
                <div className="flex items-center gap-3">
                  <div className="flex-1">
                    ...
                    <select value={exactHour} onChange={e => setExactHour(Number(e.target.value))}
                      className="w-full bg-white border-2 border-[#D5CFF0] rounded-xl px-3 py-2.5 text-[#2D2D2D] text-sm focus:border-[#8B7EC8] outline-none">
            ...
            <div className="flex gap-2">
              <button type="button" onClick={() => setIsLunar(false)}
                className={`flex-1 py-2.5 rounded-xl text-sm font-medium transition-all ${
                  !isLunar ? 'bg-[#8B7EC8] text-white shadow-md' : 'bg-white border-2 border-[#E8E2DC] text-[#9B9B9B]'
                }`}>☀️ 양력</button>
          ...
          <div>
            ...
            <div className="flex gap-2">
              <button type="button" onClick={() => setGender('남')}
                className={`flex-1 py-2.5 rounded-xl text-sm font-medium transition-all ${
                  gender === '남' ? 'bg-[#5B8AF0] text-white shadow-md' : 'bg-white border-2 border-[#E8E2DC] text-[#9B9B9B]'
                }`}>👨 남</button>
              <button type="button" onClick={() => setGender('여')}
                className={`flex-1 py-2.5 rounded-xl text-sm font-medium transition-all ${
                  gender === '여' ? 'bg-[#F06292] text-white shadow-md' : 'bg-white border-2 border-[#E8E2DC] text-[#9B9B9B]'
                }`}>👩 여</button>
            </div>
          </div>
          ...
          <button type="submit" disabled={loading}
            ...
            style={{ backgroundColor: '#D8986C', fontFamily: 'Jua, sans-serif' }}>
            {loading ? (
        ...
        <div className="space-y-6 animate-fade-in">
          ...
          <div className="card text-center">
            <div className="flex justify-center gap-3 flex-wrap mb-4">
              <div className="bg-[#FAF7F4] border-2 border-[#E8E2DC] rounded-xl px-4 py-2">
                <p className="text-xs text-[#847A80]">생년월일</p>
                <p className="text-base font-bold text-[#3D3338]">{result.birthDate}</p>
              </div>
              <div className="bg-[#FAF7F4] border-2 border-[#E8E2DC] rounded-xl px-4 py-2">
                <p className="text-xs text-[#847A80]">나이</p>
                <p className="text-base font-bold text-[#3D3338]">{result.age}세</p>
              </div>
              <div className="bg-[#FAF7F4] border-2 border-[#E8E2DC] rounded-xl px-4 py-2">
                <p className="text-xs text-[#847A80]">연간지</p>
                <p className="text-base font-bold text-[#3D3338]">{result.yearGanji} ({result.ddi}띠)</p>
              </div>
              <div className="bg-[#FAF7F4] border-2 border-[#E8E2DC] rounded-xl px-4 py-2">
                <p className="text-xs text-[#847A80]">일간 오행</p>
            ...
            {result.pillars && (
              <div className="mb-4">
                <p className="text-xs text-[#847A80] mb-2">내 사주 원국</p>
                <div className="grid grid-cols-4 gap-2 max-w-xs mx-auto">
                  {(['hour', 'day', 'month', 'year'] as const).map(key => (
                    <div key={key} className="bg-white border-2 border-[#E8E2DC] rounded-lg py-2 text-center">
            ...
            {/* 괘 정보 */}
            <div className="bg-[#F5F0EB] border-2 border-[#D5CEC8] rounded-2xl p-4 mb-3">
              <p className="text-xs text-[#847A80] mb-1">태수 {result.taesu} · 월건 {result.wolgeon} · 일진 {result.iljin}</p>
            ...
            <div className="inline-flex items-center gap-3 px-6 py-3 rounded-2xl"
              ...
              <div>
                ...
                <p className="text-sm text-[#5C5358]">올해의 전체 운세</p>
              </div>
            </div>
            ...
            {result.samjae && (
              <div className={`mt-4 p-3 rounded-xl border-2 ${
                result.samjae.active ? 'bg-[#FFF5F5] border-[#E8C8C8]' : 'bg-[#F0F8F2] border-[#C8E8D4]'
              }`}>
                <p className="text-sm font-bold mb-1">{result.samjae.active ? '⚠️ 삼재 해당' : '✅ 삼재 비해당'}</p>
                <p className="text-xs text-[#5C5358]">{result.samjae.description}</p>
              </div>
            )}
          </div>
          ...
          {result.gearAnalysis && result.gearAnalysis.length > 0 && (
            <div className="card" style={{ backgroundColor: '#F5F0FF', borderColor: '#D4CCE8' }}>
              ...
              <p className="text-xs text-[#847A80] mb-3">내 사주 8글자와 2026 병오(丙午)년이 어떻게 맞물리는지 보여줍니다</p>
          ...
          <div className="card" style={{ backgroundColor: '#F8F5FF', borderColor: '#D4CCE8' }}>
            ...
            <p className="text-base text-[#5C5358] leading-relaxed">{result.meaning}</p>
          </div>
          ...
          <div className="card">
            ...
            {result.deepContent && (
              <div className="bg-[#F8F5FF] border-2 border-[#D4CCE8] rounded-xl p-4 mb-4">
                <h3 className="text-sm font-bold text-[#7B6CB8] mb-2">📖 2026 병오년 심화 해석</h3>
                {result.deepContent.split('\n\n').map((para: string, i: number) => (
                  <p key={i} className="text-sm text-[#5C5358] leading-relaxed mb-2">{para}</p>
                ))}
              </div>
            )}

            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <div className="bg-[#F0F8F2] border-2 border-[#C8E8D4] rounded-xl p-3">
                <p className="text-sm text-[#5FB88A] font-bold mb-1">💡 조언</p>
                <p className="text-sm text-[#3D5838]">{result.advice}</p>
              </div>
              <div className="bg-[#FFF5F5] border-2 border-[#E8C8C8] rounded-xl p-3">
            ...
            <div className="space-y-4">
              {result.categories.map((cat: any) => (
                <div key={cat.label} className="bg-[#FAF7F4] border-2 border-[#E8E2DC] rounded-xl p-4">
                  ...
                  <p className="text-sm text-[#5C5358] leading-relaxed mb-2">{cat.description}</p>
                  <div className="bg-white/60 rounded-lg p-2 border border-[#E8E2DC]">
          ...
          <div className="card">
                ...
                <button key={mf.month}
                  ...
                  className={`p-2 rounded-xl border-2 text-center transition-all ${
                    selectedMonth === mf.month
                      ? 'bg-[#8B7EC8] border-[#7B6CB8] text-white'
                      : 'bg-[#FAF7F4] border-[#E8E2DC] hover:border-[#8B7EC8]'
                  }`}>
            ...
            {selectedMonth && (() => {
              ...
              if (!mf) return null;
              return (
                <div className="bg-[#F8F5FF] border-2 border-[#D4CCE8] rounded-xl p-4 animate-fade-in">
                  ...
                  <p className="text-base text-[#5C5358] leading-relaxed">{mf.description}</p>
                  {mf.gearNote && (
            ...
            {!selectedMonth && (
              <p className="text-center text-sm text-[#847A80]">👆 월을 클릭하면 상세 운세를 볼 수 있어요!</p>
            )}
          </div>
            ...
            <div className="flex items-end gap-1 h-40">
              {result.monthlyFortunes.map((mf: any) => (
                <div key={mf.month} className="flex-1 flex flex-col items-center justify-end h-full">
                  ...
                  <p className="text-xs text-[#847A80] mt-1">{mf.month}월</p>
                </div>
              ))}
            </div>
          </div>
