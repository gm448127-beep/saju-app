...
import type { HomePatternCard as HomePatternCardData } from "@/lib/today-pattern-helpers";
...
export default function HomePatternCard({ data }: HomePatternCardProps) {
  const { insight, subline, last7, topTones, stats, phase } = data;
  ...
  return (
    ...
    >
      ...
      <div className="mt-4 rounded-2xl border border-[#E8D7C4]/80 bg-white/75 px-3 py-3">
        <div className="flex items-center justify-between gap-2">
          ...
          <p className="text-[10px] font-semibold text-[#8A7E78]">
            {recordedCount > 0 ? `${recordedCount}일 기록` : "기록 대기"}
          </p>
        </div>
          ...
          {last7.map((day) => (
            <div key={day.dateKey} className="flex flex-col items-center gap-1">
              ...
              {day.hasRecord ? (
                ...
                <div className="flex h-[10px] w-full items-center justify-center rounded bg-[#EDE4DC]/60">
      ...
      {phase === "empty" && (
        <p className="mt-3 rounded-xl border border-dashed border-[#D9C8C0] bg-white/60 px-3 py-2 text-[11px] leading-relaxed text-[#6B5E58]">
          매일의 결이 쌓일수록 나만의 흐름이 선명해집니다
        </p>
      )}
